...
import com.reign.framework.jdbc.async.SqlFormatter;
...
public final class AsyncDBExecutor {
     ...
     * @param interval
     */
    public void addSQL(AsyncOp op, JdbcEntity entity, String idKey, Object obj, int order, long interval) {
        SQLEntry entry = new SQLEntry();
        entry.id = order;
        entry.aliveTime = interval;
        entry.op = AsyncOp.UPDATE;
     ...
     * @param interval
     */
    public void addUpdateSQL(JdbcEntity entity, String idKey, Object obj, Object old, int order, long interval) {
        SQLEntry entry = new SQLEntry();
        entry.id = order;
        entry.aliveTime = interval;
        entry.op = AsyncOp.UPDATE;
     ...
     * @return
     */
    private String[] getUpdateSQL(Object obj, JdbcEntity entity) {
        String sqlIdentiry = entity.getUpdateSQL();
        sqlIdentiry = sqlIdentiry.trim();
        String sql = SqlFormatter.format(sqlIdentiry, entity.builderUpdateParams(obj));
        return new String[]{sqlIdentiry, sql};
    }
     ...
     * @return
     */
    private String[] getDynamicUpdateSQL(Object newObj, Object oldObj, JdbcEntity entity) {
        //如果不支持动态更新，则生成普通SQL
        if (!(newObj instanceof IDynamicUpdate) || !(oldObj instanceof JdbcModel)) {
            return getUpdateSQL(newObj, entity);
        }

        //和旧对象对比，动态生成SQL
        IDynamicUpdate newUpdateObj = (IDynamicUpdate) newObj;
        JdbcModel oldUpdateObj = (JdbcModel) oldObj;

        final Tuple<String, List<Param>> tuple = newUpdateObj.dynamicUpdateSQL(entity.getTableName(), oldUpdateObj);
        ...
        if (StringUtils.isNotBlank(tuple.left)) {
            ...
            Object[] idValues = entity.getId().getIdValue(newObj);
            for (Object obj : idValues) {
                tuple.right.add(new Param(obj));
            }
            return new String[]{builder.toString(), SqlFormatter.format(builder.toString(), tuple.right)};
        }
        return getUpdateSQL(newObj, entity);
    }
